...
function JobOffersTable({currentUser}) {
    ...
    const fetchJobOffers = async () => {
        setLoading(true);
        try {
            const data = await JobOfferAPI.GetJobOffers(new JobOfferFilter(null, null, filter.status), new Pagination(page - 1, 8));
            setJobOffers(data);
        } catch (error) {
    ...
    return (
        <>
            ...
            {!editJobOffer ?
                ...
                :
                <JobOfferForm jobOffer={editingJobOffer} customer={customer} currentUser={currentUser}></JobOfferForm>
            }
        </>
    )
}
